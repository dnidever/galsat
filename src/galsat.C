            ...
            real dt_tot, bool init_out, bool x_flag, bool mw_flag)
{
    ...
    get_acc_jerk_pot_coll(mass, pos, vel, acc, jerk, n, epot, coll_time, mw_flag);
    ...
    while (true){
	    ...
	    real dt = -dt_param * coll_time;          // dt<0, dt_param, coll_time >0
            evolve_step(mass, pos, vel, acc, jerk, n, t, dt, epot, coll_time, mw_flag);
            nsteps++;
        }
        if (t <= t_dia){             // t<0
