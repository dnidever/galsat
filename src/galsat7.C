...
void evolve(const real mass[], const real soft[], real pos[][NDIM], real vel[][NDIM],
            ...
            real pcol[], real lastr[2], int & napo);
void evolve_step(const real mass[], const real soft[], real pos[][NDIM], real vel[][NDIM],
                 ...
                 const real vcirc, const real ds, real pcol[], real lastr[2], int & napo);
...
void put_snapshot(const real mass[], const real soft[], const real pos[][NDIM],
                  ...
                  const real acc[][NDIM], const real jerk[][NDIM], const int nput, const real pcol);
...
void unbound(const real mass[], const real soft[],const real pos[][NDIM], real vel[][NDIM], 
             int n, real pcol[], real lastr[2], int & napo);
...
int main(int argc, char *argv[])
{
    ...
    real * pcol = new real[n];                  // color flag for u nbound time of each particle
    real (* pos)[NDIM] = new real[n][NDIM];     // positions for all particles
    real (* vel)[NDIM] = new real[n][NDIM];     // velocities for all particles

    real * lastr = new real[2];                  // distance of satellite to GC at last two timesteps
    lastr[0] = 0;                                // assumes first particle is satellite
    lastr[1] = 0;

    int napo = 0;                               // which apo are we on?
    ...
    evolve(mass, soft, pos, vel, n, t, dt_param, dt_dia, dt_out, dt_tot, init_out,
           x_flag, mw_flag, df_flag, vcirc, ds, tstep, dtmin, dtmax, tsign, pcol, lastr, napo);
            ...
            real pcol[], real lastr[2], int & napo)
{
    ...
    if (init_out)                                    // flag for initial output
       put_snapshot(mass, soft, pos, vel, n, t, acc, jerk, 0, pcol);
    ...
    while (true){
	    ...
	    //dt = -0.0001;
            evolve_step(mass, soft, pos, vel, acc, jerk, n, t, dt, epot, coll_time, mw_flag, df_flag, vcirc, ds, pcol, lastr, napo);
            nsteps++;
        }
        ...
        if (tsign*t >= tsign*t_out){              // tsign*t>0
            nput++;
            put_snapshot(mass, soft, pos, vel, n, t, acc, jerk, nput, pcol);
                 ...
                 const real vcirc, const real ds, real pcol[], real lastr[2], int & napo)
{
    ...
    unbound(mass,soft,pos,vel,n,pcol,lastr,napo);
...
/*-----------------------------------------------------------------------------
 *  unbound -- figure out at which apo each particle becomes unbound from
             ...
             int n, real pcol[], real lastr[2], int & napo)
{
   ...
   bool apo = false;             // Are we at apo?  False until proven true

   real rsat = pos[0][0]*pos[0][0]+pos[0][1]*pos[0][1]+pos[0][2]*pos[0][2];
   rsat = sqrt(rsat);

   real epot = 0;
   real ekin = 0;

   // IS THIS APO?, false unless proved true
   if ( (lastr[1] >= lastr[0]) && (rsat<lastr[1]) && (lastr[0]>0) && (lastr[1]>0) ) {
          apo = true;          // make apo true
          napo++;              // increment napo
   }   



   // IS THE PARTICLE UNBOUND?

   // Assuming first body is satellite
   // We're at apo
   if (apo) {


     int i = 0;    // Assume the first particle is the satellite
     ...
     for (int j = 1; j < n ; j++) {
        ...
        // Energy per mass, since the test particle has no mass
        epot = -G * mass[i] / sqr2e2;
        ekin = 0.5 * v2;

       // If energy>=0 then UNBOUND
       // Checking to make sure it hasn't gone unbound on a previous apo
	if ( (epot+ekin >= 0) && (pcol[j]==0) ) {
	  pcol[j] = napo;
        }
   ...
   //cerr << napo << endl;
   //cerr  << lastr[0] << ' ' << lastr[1] << ' ' << rsat << ' ' << napo << endl;
