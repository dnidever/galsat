pro plotnbody,file,dfile,over=over,twoplot=twoplot,arr=arr,stp=stp,$
...
; Not enough inputs
if n_elements(file) eq 0 and n_elements(arr) eq 0 then begin
...
if (n_elements(arr(0,0,*)) gt 8) then begin
  dum = arr
  arr = arr(*,*,0:7)*0.
  arr(*,*,0:1) = dum(*,*,0:1)
  arr(*,*,2:7) = dum(*,*,3:8)
...
dum = closest(0.0,reform(arr(*,0,0)),ind=cur)
