...
void evolve(const real mass[], const real soft[], real pos[][NDIM], real vel[][NDIM],
            ...
            const real vcirc, const real ds, const real vhalo, real tstep, real dtmin, real dtmax, int tsign);
void evolve_step(const real mass[], const real soft[], real pos[][NDIM], real vel[][NDIM],
                 ...
                 const real vcirc, const real ds, const real vhalo);
void get_acc_jerk_pot_coll(const real mass[], const real soft[], const real pos[][NDIM],
                           ...
                           const real ds, const real vhalo);
...
int main(int argc, char *argv[])
{
    ...
    evolve(mass, soft, pos, vel, n, t, dt_param, dt_dia, dt_out, dt_tot, init_out,
           x_flag, mw_flag, df_flag, vcirc, ds, vhalo, tstep, dtmin, dtmax, tsign);
            ...
            const real vcirc, const real ds, const real vhalo, real tstep, real dtmin, real dtmax, int tsign)
{
    ...
    get_acc_jerk_pot_coll(mass, soft, pos, vel, acc, jerk, n, epot, coll_time, mw_flag, df_flag, vcirc, ds, vhalo);
    ...
    while (true){
	    ...
	    //dt = -0.0001;
            evolve_step(mass, soft, pos, vel, acc, jerk, n, t, dt, epot, coll_time, mw_flag, df_flag, vcirc, ds, vhalo);
            nsteps++;
        }
