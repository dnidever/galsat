pro plotseparr5,save=save,color=color,post=post
...
;  image = max(image)+min(image)-image   ; flip it
 ...
 ;  position = [0.1,0.1,0.9,0.9]
   ...
   if not keyword_set(post) then im = CONGRID(image, xsize, ysize,/interp) $
     else im = congrid(image,nx*20 < 700,ny*20 < 700,/interp)
...
;	XSize=dev_pos(2)-dev_pos(0), YSize=dev_pos(3)-dev_pos(1)
  ...
  if keyword_set(save) then begin
    ...
    file = 'plotseparr5_'+num+'.tiff'

    ; creating tiff image
    tiff = tvrd(true=1)
    tiff = reverse(tiff,3)
    write_tiff,'animsep5/'+file,tiff,1
  endif
