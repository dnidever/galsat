pro plotseparr6,file,save=save,color=color,post=post,$
                single=single,sub=sub,top=top,$
                vcirc=vcirc,anim=anim,savefile=savefile
...
;if not keyword_set(save) then window,xsize=640*1.2,ysize=512*1.2
fac=1.3
if not keyword_set(save) then window,xsize=1000.,ysize=650    ;default
...
WHILE (flag ne 1) do begin
  ...
  if not keyword_set(anim) and keyword_set(vcirc) then begin
    v=closest(vcirc,vcircarr,ind=i)
  endif

  if not keyword_set(savefile) then savefile='plotseparr6'
  if keyword_set(save) then ps_open,savefile,color=color
  ...
  alltit = '!3V!dcirc!n = '+stringize(vcircarr(i),ndec=2)+' km s!u-1!n'
  ...
  charsize=1.5
  if keyword_set(save) and not keyword_set(single) then charsize=charsize*0.65
...
;  image = max(image)+min(image)-image   ; flip it
 ...
 ;  position = [0.1,0.1,0.9,0.9]
   ...
   interp = 0
   if not keyword_set(save) then im = CONGRID(image, xsize, ysize,interp=interp) $
...
;	XSize=dev_pos(2)-dev_pos(0), YSize=dev_pos(3)-dev_pos(1)
  ...
  if not keyword_set(save) then Device, Decomposed=0   ; turns display color on
  TV, byte_im, device=device, normal=normal, xstart, ystart, $
        XSize=xsize, YSize=ysize
  if not keyword_set(save) then Device, Decomposed=1   ; turns display color off
  ...
  ; if keyword_set(save) then colpos = [0.1,0.85,0.95,0.90]
  ...
  if NOT keyword_set(single) then begin
   ...
   decomp = 1
   if keyword_set(save) then decomp=0
   display,reform(tarr(i,*,*)),xs,ys,pos=pos2,/noerase,decomp=decomp,$
 ...
 ;  xyouts,0.4,0.95,alltit,charsize=charsize*1.5,/normal
  ...
  if keyword_set(anim) then begin
    ...
    file = 'plotseparr6d_'+num+'.tiff'

    ; creating tiff image
    tiff = tvrd(true=1)
    tiff = reverse(tiff,3)
    write_tiff,'animsep6d/'+file,tiff,1
  endif
 ...
 ;   ;spawn,'cp idl.ps animsep2/'+file
  ...
  if keyword_set(anim) and i eq nz-1 then flag=1
