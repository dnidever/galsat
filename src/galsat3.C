            ...
            const real ds)
{
    ...
    get_acc_jerk_pot_coll(mass, pos, vel, acc, jerk, n, epot, coll_time, mw_flag, vcirc, ds);
    ...
    while (true){
	    ...
	    real dt = -dt_param * coll_time;          // dt<0, dt_param, coll_time >0
            evolve_step(mass, pos, vel, acc, jerk, n, t, dt, epot, coll_time, mw_flag, vcirc, ds);
            nsteps++;
        }
        if (t <= t_dia){             // t<0
