...
void evolve(const real mass[], const real soft[], real pos[][NDIM], real vel[][NDIM],
            ...
            const real ds, real tstep, real dtmin, real dtmax, int tsign);
...
bool read_options(int argc, char *argv[], real & dt_param, real & dt_dia,
                  ...
                  bool & mw_flag, real & tstep, real & dtmin, real & dtmax, int & tsign);
...
int main(int argc, char *argv[])
{
    ...
    if (! read_options(argc, argv, dt_param, dt_dia, dt_out, dt_tot, init_out,
                       x_flag, mw_flag, tstep, dtmin, dtmax, tsign))
    ...
    evolve(mass, soft, pos, vel, n, t, dt_param, dt_dia, dt_out, dt_tot, init_out,
           x_flag, mw_flag, vcirc, ds, tstep, dtmin, dtmax, tsign);
...
bool read_options(int argc, char *argv[], real & dt_param, real & dt_dia,
                  ...
                  real & tstep, real & dtmin, real & dtmax, int & tsign)
{
    int c;
    while ((c = getopt(argc, argv, "hd:e:o:t:s:l:c:ixmf")) != -1)
        switch(c){
                  ...
                  const real acc[][NDIM], const real jerk[][NDIM], const int nput)
{
    ...
    // Nbodies >= 10000, outputting to a file
    if (n >= 1000) {
    ...
    // Nbodies < 10000, outputting to stdout
    } else {
            ...
            const real ds, real tstep, real dtmin, real dtmax, int tsign)
{
    ...
    get_acc_jerk_pot_coll(mass, soft, pos, vel, acc, jerk, n, epot, coll_time, mw_flag, vcirc, ds);
    ...
    while (true){
	    ...
	    //dt = -0.0001;
            evolve_step(mass, soft, pos, vel, acc, jerk, n, t, dt, epot, coll_time, mw_flag, vcirc, ds);
            nsteps++;
        }
	...
	if (mass[i] != 0.0) {
	  // cerr << mass[i] << " " << i << endl;
	   ...
	   const real rs = 0.7;    // c in paper
        //   const real c = 13.0;     // now ds, d in paper (12.0 kpc in paper)
           const real q = 0.9;
           const real p = 1.0;
