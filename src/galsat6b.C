...
void evolve(const real mass[], const real soft[], real pos[][NDIM], real vel[][NDIM],
            ...
            const real vcirc, const real ds, real tstep, real dtmin, real dtmax, int tsign, real galepot[]);
void evolve_step(const real mass[], const real soft[], real pos[][NDIM], real vel[][NDIM],
                 ...
                 const real vcirc, const real ds, real galepot[]);
...
int main(int argc, char *argv[])
{
    ...
    real * galepot = new real[n];               // potential energy for each all particles, just from galaxy

    get_snapshot(mass, soft, pos, vel, n);

    evolve(mass, soft, pos, vel, n, t, dt_param, dt_dia, dt_out, dt_tot, init_out,
           x_flag, mw_flag, df_flag, vcirc, ds, tstep, dtmin, dtmax, tsign, galepot);
   ...
   //      << (etot - einit) / einit << endl;
    ...
    if (x_flag){
        ...
        for (int i = 0; i < n ; i++){
            ...
            real ekin1 = 0;
            for (int k = 0; k < NDIM ; k++)
              ekin1 += 0.5 * vel[i][k] * vel[i][k];
	    // ekin1 += 0.5 * mass[i] * vel[i][k] * vel[i][k];
            cerr << ' ' << galepot[i] << ' ' << ekin1 << ' ' << galepot[i]+ekin1; 
            cerr << endl;
        }
    }
}
            ...
            const real vcirc, const real ds, real tstep, real dtmin, real dtmax, int tsign, real galepot[])
{
    ...
    get_acc_jerk_pot_coll(mass, soft, pos, vel, acc, jerk, n, epot, coll_time, mw_flag, df_flag, vcirc, ds, galepot);
    ...
    while (true){
	    ...
	    //dt = -0.0001;
            evolve_step(mass, soft, pos, vel, acc, jerk, n, t, dt, epot, coll_time, mw_flag, df_flag, vcirc, ds, galepot);
            nsteps++;
        }
        if (tsign*t >= tsign*t_dia){             // tsign*t>0
            write_diagnostics(mass, soft, pos, vel, acc, jerk, n, t, epot, nsteps,
                              einit, false, x_flag, galepot);
                           ...
                           const real ds, real galepot[])
{
  ...
  //  even if it's a test particles, b/w otherwise it won't "feel" the MW potential
	...
	// galepot[i]=mass[i]*(phih+phim+phis);
	galepot[i]=(phih+phim+phis);
